...
var d3Array_5 = d3Array.range;
var d3Array_6 = d3Array.bisect;
var d3Array_7 = d3Array.tickStep;
var d3Array_8 = d3Array.ticks;
var d3Array_9 = d3Array.tickIncrement;
var d3Array_10 = d3Array.ascending;
...
var d3Color_1 = d3Color.cubehelix;
var d3Color_2 = d3Color.rgb;
...
var d3Interpolate_1 = d3Interpolate.interpolate;
var d3Interpolate_2 = d3Interpolate.interpolateNumber;
var d3Interpolate_3 = d3Interpolate.interpolateRound;
var d3Interpolate_4 = d3Interpolate.piecewise;
var d3Interpolate_5 = d3Interpolate.interpolateRgbBasis;
var d3Interpolate_6 = d3Interpolate.interpolateCubehelixLong;
...
function polymap(domain, range, interpolate) {
  ...
  return function(x) {
    var i = d3Array_6(domain, x, 1, j) - 1;
    return r[i](d[i](x));
  };
}
...
function transformer() {
  ...
  scale.invert = function(y) {
    return clamp(untransform((input || (input = piecewise(range, domain.map(transform), d3Interpolate_2)))(y)));
  };
  ...
  scale.rangeRound = function(_) {
    return range = Array.from(_), interpolate = d3Interpolate_3, rescale();
  };
...
var d3Format_2 = d3Format.formatSpecifier;
var d3Format_3 = d3Format.precisionPrefix;
var d3Format_4 = d3Format.formatPrefix;
var d3Format_5 = d3Format.precisionRound;
var d3Format_6 = d3Format.precisionFixed;

function tickFormat(start, stop, count, specifier) {
  var step = d3Array_7(start, stop, count),
      precision;
  specifier = d3Format_2(specifier == null ? ",f" : specifier);
  switch (specifier.type) {
    case "s": {
      var value = Math.max(Math.abs(start), Math.abs(stop));
      if (specifier.precision == null && !isNaN(precision = d3Format_3(step, value))) specifier.precision = precision;
      return d3Format_4(specifier, value);
    }
    case "":
    case "e":
    case "g":
    case "p":
    case "r": {
      if (specifier.precision == null && !isNaN(precision = d3Format_5(step, Math.max(Math.abs(start), Math.abs(stop))))) specifier.precision = precision - (specifier.type === "e");
      break;
    }
    case "f":
    case "%": {
      if (specifier.precision == null && !isNaN(precision = d3Format_6(step))) specifier.precision = precision - (specifier.type === "%") * 2;
      break;
    }
  }
  return d3Format_1(specifier);
}
...
function linearish(scale) {
  ...
  scale.ticks = function(count) {
    var d = domain();
    return d3Array_8(d[0], d[d.length - 1], count == null ? 10 : count);
  };
  ...
  scale.nice = function(count) {
    ...
    step = d3Array_9(start, stop, count);
...
var d3Dispatch_1 = d3Dispatch.dispatch;
var d3Dispatch_2 = d3Dispatch.touch;
...
var d3Selection_4 = d3Selection.event;
var d3Selection_5 = d3Selection.touch;
var d3Selection_6 = d3Selection.customEvent;

function nopropagation() {
  d3Selection_4.stopImmediatePropagation();
}

function noevent() {
  d3Selection_4.preventDefault();
  d3Selection_4.stopImmediatePropagation();
}

function nodrag(view) {
  var root = view.document.documentElement,
      selection = d3Selection_1(view).on("dragstart.drag", noevent, true);
...
function yesdrag(view, noclick) {
  var root = view.document.documentElement,
      selection = d3Selection_1(view).on("dragstart.drag", null);
  if (noclick) {
...
function defaultFilter() {
  return !d3Selection_4.ctrlKey && !d3Selection_4.button;
}
...
function defaultSubject(d) {
  return d == null ? {x: d3Selection_4.x, y: d3Selection_4.y} : d;
}
...
function drag() {
  var filter = defaultFilter,
      ...
      gestures = {},
      listeners = d3Dispatch_1("start", "drag", "end"),
      active = 0,
      mousedownx,
      mousedowny,
      mousemoving,
  ...
  function mousedowned() {
    ...
    var gesture = beforestart("mouse", container.apply(this, arguments), d3Selection_3, this, arguments);
    if (!gesture) return;
    d3Selection_1(d3Selection_4.view).on("mousemove.drag", mousemoved, true).on("mouseup.drag", mouseupped, true);
    nodrag(d3Selection_4.view);
    nopropagation();
    mousemoving = false;
    mousedownx = d3Selection_4.clientX;
    mousedowny = d3Selection_4.clientY;
    gesture("start");
  }
  ...
  function mousemoved() {
    noevent();
    if (!mousemoving) {
      var dx = d3Selection_4.clientX - mousedownx, dy = d3Selection_4.clientY - mousedowny;
  ...
  function mouseupped() {
    d3Selection_1(d3Selection_4.view).on("mousemove.drag mouseup.drag", null);
    yesdrag(d3Selection_4.view, mousemoving);
    noevent();
    gestures.mouse("end");
  }
  ...
  function touchstarted() {
    ...
    var touches = d3Selection_4.changedTouches,
        c = container.apply(this, arguments),
        n = touches.length, i, gesture;

    for (i = 0; i < n; ++i) {
      if (gesture = beforestart(touches[i].identifier, c, d3Selection_5, this, arguments)) {
        nopropagation();
        gesture("start");
      }
    }
  ...
  function beforestart(id, container, point, that, args) {
    ...
    if (!d3Selection_6(new DragEvent(drag, "beforestart", s, id, active, p[0], p[1], 0, 0, sublisteners), function() {
      if ((d3Selection_4.subject = s = subject.apply(that, args)) == null) return false;
      dx = s.x - p[0] || 0;
      dy = s.y - p[1] || 0;
      return true;
    })) return;
    ...
    return function gesture(type) {
      ...
      d3Selection_6(new DragEvent(drag, type, s, id, n, p[0] + dx, p[1] + dy, p[0] - p0[0], p[1] - p0[1], sublisteners), sublisteners.apply, sublisteners, [type, that, args]);
    };
  }
...
function line() {
  ...
  function line(data) {
    ...
    if (context == null) output = curve(buffer = d3Path_1());
...
function symbol() {
  ...
  function symbol() {
    var buffer;
    if (!context) context = buffer = d3Path_1();
  ...
  L.Control.Heightgraph = L.Control.extend({
    ...
    */
    ...
    _computeStats: function _computeStats() {
      var max$1 = this._profile.maxElevation = max(this._profile.elevations);
      var min$1 = this._profile.minElevation = min(this._profile.elevations);
      var quantile$1 = this._profile.elevationQuantile = quantile(this._profile.elevations, 0.75);
      this._profile.yElevationMin = quantile$1 < min$1 + min$1 / 10 ? min$1 - max$1 / 5 < 0 ? 0 : min$1 - max$1 / 5 : min$1 - max$1 / 10;
      this._profile.yElevationMax = quantile$1 > max$1 - max$1 / 10 ? max$1 + max$1 / 3 : max$1;
    },
    ...
    _appendScales: function _appendScales() {
      ...
      this._x = linear().range([0, width]);
      this._y = linear().range([height, 0]);
...
var L_Control_Heightgraph = L.control.heightgraph;

module.exports = L_Control_Heightgraph;
